...
var wtf = {
  get util() { return (exports.util = require('./fuckparse/util')); }
};
exports.util = wtf.util;
...
var color = {
  codes : {'bright':1,'blink':5,'red':31,'green':32,'yellow':33,
  ...
  methods : {
    ...
    // },
    color : function(str) {
      if ('none'==arguments[1] && 2 == arguments.length) return str;
      var these = [], c;
      for (var i = 1; i < arguments.length; i++) {
        if ('object' == typeof arguments[i]) {
          for (var j = 0; j < arguments[i].length; j++) {
            // un-dry at expense of a small performance gain over flatten()
            (c = color.codes[arguments[i][j]]) && these.push(c.toString());
          }
        } else {
          (c = color.codes[arguments[i]]) && these.push(c.toString());
        }
      }
      return '\u001b[' + these.join(';') + 'm' + str + '\u001b[0m';
    }
  }
};
...
table.Table = function(rows, colOptsArray) {
  this.rows = rows;
  this.colOpts = colOptsArray || [];
  this.rowSeparator = '';
  this.padCharacter = ' ';
};
table.Table.prototype = {
  ...
  renderTo : function(cout) {
    this.cout = cout.puts ? cout : putsify(cout);
    var me = this;
    this.eachPrerenderedRow(function(cols){
      me.cout.puts(cols.join(me.rowSeparator));
    });
  },
  eachPrerenderedRow : function(f) {
    ...
    for (var i = 0, j = this.rows.length; i < j; i++) {
      ...
      var rowRender = new Array(row.length - 1);
      for (var k = 1; k < row.length; k++ ) {
        ...
        var pad = add ? Array(add+1).join(this.padCharacter) : '';
  ...
  buildPrerenderedRows : function() {
    var r = [];
    this.eachPrerenderedRow(function(cols){ r.push(cols); });
    return r;
  },
...
table.methods.render = function(rows, colsOpts, cout) {
  (new table.Table(rows, colsOpts)).renderTo(cout);
};

var putsify = exports.putsify = function(stream, force) {
  if (!stream['puts'] || force) {
...
OptionalParameter.prototype = {
  ...
  _takeFunction : function(f) {
    if (this._f) this._onFunctionCollision(f);
    else this._f = f;
  },
  _onFunctionCollision : function(f) {
  ...
  _processOpts : function(o) {
    ...
    for (i in o) switch (i) {
      ...
      case 'default' :
        ...
        break;
      case 'enum' :
        this._processEnum(o['enum'], o);
        break;
      case 'enumDefault' :
        this.enumDefault = o[i];  // haha w/e it's so easy
        break;
      case 'ambiguous' :
      case 'invalid'   :
        ...
        break;
      default:
        throw new SyntaxSyntaxError('bad option option: "' + i + '"');
    }
  },
  // @todo break up after a test lockdown
  _processEnum : function(enumVals) {
    if (!this.takesArgument()) throw new SyntaxSyntaxError("cannot define"+
    " enum unless the parameter takes an argument.");
    if ('object' != typeof enumVals) throw new SyntaxSyntaxError(
      'Expecting array for enum value, not "' + enumVals + '"');
    if (this._f) this.__afterEnum = this._f;
    var origCollide = this._onFunctionCollision;
    this._onFunctionCollision = function(f) {
      if (this.__afterEnum) origCollide(f);
      else this.__afterEnum = f;
    };
    this.__enumVals = enumVals;
    var optDef = this;
    this['enum'] = function() {
      return exports.util.abbrevesqueRender(exports.util.abbrevesque(
        optDef.__enumVals
      ));
    };
    this._f = function(x) {
      if (true == x && ! optDef.argumentIsRequired() &&
        undefined != optDef.enumDefault) x = optDef.enumDefault;
      var ctx = this, m = (optDef.__msgs || {});
      var use = exports.util.normalizeEnum(x, enumVals, {
        ambiguous : (m.ambiguous ||
          'Ambiguous value {given} for {param}.  Did you mean {similar}?'),
        invalid :   (m.invalid ||
          'Expecting {all} not {given} for {param}.'),
        templateVars : { param : function(){ return optDef.casual(); } }
      }, function(e) {
        ctx.out.puts(e.toString());
        ctx.out.puts(ctx.parse.cmd.invite(ctx));
      });
      if (!use) return false;
      if (optDef.isRepeatable()) {
        var arr = this.request.values[optDef.intern()];
        arr[arr.length - 1] = use;
      } else {
        this.request.values[optDef.intern()] = use;
      }
      if (optDef.__afterEnum) return optDef.__afterEnum(use);
    };
  },
  ...
  _setLong : function(stem, takesArg, argRequired, argLabel, syntaxTail){
    ...
    // we want to allow a short flag with no arg and a long flag w/ opt. arg
    if (undefined != (x = this._takesArgument) && x != takesArg && argRequired)
 ...
 */
var ExecutionContext = exports.ExecutionContext = function(o, e) {
  this.out = o || putsify(process.stdout);
  this.err = e || putsify(process.stderr);
};
...
Command.prototype = {
  ...
  strong : function(s) { return this.color(s, 'bright', 'green'); },
