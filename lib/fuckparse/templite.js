...
Templite.prototype = {
  ...
  run : function(dataSource) {
    ...
    for (var i = 0; i < this._sexp.length; i ++) {
      if ('string' == this._sexp[i][0]) {
        ...
        parts[i] = this._getValue(this._sexp[i][1]);
      }
    }
    return parts.join('');
  },
  ...
  _getValue : function(name) {
    var useVal, found = false;
    for (var i = 0; i < this.__datasources.length; ++i) {
      var val = this.__datasources[i][name];
      // we assume that a user-provided value of null or undefined
      // is the same as having not provided a value.
      if (undefined != val && null != val) {
        if ('function' == typeof val) {
          found = true;
          useVal = val.call(this.__datasources[i]);
        } else {
          found = true;
          useVal = val;
        }
...
if (process.argv && process.argv[1] == __filename) {
  ...
  // start tests for multiple datasources
  var t4 = Templite.build('{a}beta{c}');
  equal(t4.run(), 'beta', 'no datasources'); // secret who hah
  equal(t4.run({a : 'ALPHA'}), 'ALPHAbeta', 'one datasource');
  equal(t4.run({z:'x'}, {z:'y'}), 'beta', 'two bad datasources');
  equal(t4.run({a:'wing'}, {c:'ding'}), 'wingbetading', 'two good datasources');
  equal(t4.run({a:'YES'}, {a:'NO'}, {c:'HELLO'}), 'YESbetaHELLO', 'foo');
  // end tests for multiple datasources
