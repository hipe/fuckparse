var lib = require('../fuckparse'),
   util = require('./util');

lib.ExecutionContext.prototype.pushCommandToStack = function(cmd, asUsed) {
  this.commandStack().push([cmd, asUsed]);
};
...
lib.Parse.prototype.addCommand = function(cmd, asUsed) {
  if (this.asUsed) { // a bit confused
    this.ec.pushCommandToStack(this.cmd, this.asUsed);
  }
  this.cmd = cmd;
  this.asUsed = asUsed;
  return this;
...
var Subcommand = exports.Subcommand = {
  ...
  syntaxString : function(s) {
    return (s) ? (this.__synstr = s) :  (this.__synstr || this._intern);
  },
  label : function()        { return this._intern; },
  execute : function(f) {
  ...
  _invocationString : function(ctx) {
    var tox = [ctx.programName()], these = ctx.commandStack();
    for (var i = 0; i < these.length; i++) tox.push(these[i][1]); // asUsed
    if (ctx.parse.asUsed) tox.push(ctx.parse.asUsed); // maybe officious
    return tox.join(' ');
  },
  ...
  _determineTerminalCommand : function(request, ctx) {
    ...
    var childRequest = ctx.parse.addCommand(cmd, subName).parseCommand();
  ...
  _fuzzyMatch : function(subName, ctx) {
    var re = new RegExp('^' + util.regexpEscape(subName));
    var matches = [];
  ...
  handleSubcommandNotFound : function(subName, ctx) {
    return this._handleSubcommandNotFound(subName, ctx);
  },
  _handleSubcommandNotFound : function(subName, ctx) {
